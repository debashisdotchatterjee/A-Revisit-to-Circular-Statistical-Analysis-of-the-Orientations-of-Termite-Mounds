...
output_dir <- "termite_3orientation_analysis"
...
analyze_circular_data <- function(dataset, set_name) {
  ...
  plot_data <- ggplot() +
    ...
    geom_line(data = density_df, aes(x = theta, y = density), 
              color = "red", size = 1.2) +  # Overlay scaled von Mises density
    ggtitle(plot_title) +
    theme_minimal() +
    xlab("Angle (degrees)") +
    ylab("Density")
...
# Create a directory to save all outputs
output_dir <- "termite_orientation33_analysis"
if (!dir.exists(output_dir)) {
  dir.create(output_dir)
}
...
analyze_circular_data <- function(dataset, set_name) {
  ...
  # Save the plot
  save_output(plot_data, paste0(set_name, "_circular_plot_von_mises.png"))
  
  # Save plot without von Mises overlay for comparison
  ...
  # Save the plot without von Mises fit
...
# Function to fit triangular distribution and calculate density
triangular_density <- function(theta, mu) {
  # Triangular distribution formula for circular data
  1/(2 * pi) * (1 + 2 * cos(theta - mu))
}

# Function to analyze each dataset in fisherB13c
analyze_circular_data_triangular <- function(dataset, set_name) {
  
  # Convert dataset to radians for the analysis
  data_rad <- circular(dataset, units = "degrees")
  
  # Estimate mean direction (mu) for triangular distribution
  mean_dir <- mean.circular(data_rad)
  circ_var <- var.circular(data_rad)
  ...
  # Generate the triangular density curve based on estimated mean direction (mu)
  theta_seq <- seq(0, 2 * pi, length.out = 360)  # Sequence of angles in radians
  density_values <- triangular_density(theta_seq, mean_dir)  # Compute density
  ...
  # Plot the circular data with triangular distribution density overlaid
  plot_title <- paste("Circular Plot with Triangular Fit for", set_name)
  plot_data <- ggplot() +
    geom_histogram(aes(x = as.numeric(data_rad)), 
                   binwidth = 10, fill = "lightblue", color = "black", 
                   position = "identity", alpha = 0.6) +
    coord_polar(theta = "x") +
    geom_line(data = density_df, aes(x = theta, y = density), 
              color = "red", size = 1.2) +  # Overlay scaled triangular density
    ggtitle(plot_title) +
    theme_minimal() +
    xlab("Angle (degrees)") +
    ylab("Density")
  
  # Save the plot with triangular distribution overlay
  save_output(plot_data, paste0(set_name, "_circular_plot_triangular.png"))
  
  # Save plot without triangular overlay for comparison
  plot_no_density <- ggplot() +
    geom_histogram(aes(x = as.numeric(data_rad)), 
                   binwidth = 10, fill = "lightblue", color = "black", 
                   position = "identity", alpha = 0.6) +
  ...
  # Save the plot without triangular distribution fit
  save_output(plot_no_density, paste0(set_name, "_circular_plot_no_density.png"))
...
all_summaries <- lapply(names(fisherB13c), function(set_name) {
  dataset <- fisherB13c[[set_name]]
  analyze_circular_data_triangular(dataset, set_name)
})
...
# Generate a table of all the model fits and save it
combined_summary_plot <- ggplot(combined_summary, aes(x = Set, y = Mean_Direction)) +
  geom_bar(stat = "identity", fill = "lightgreen", color = "black") +
  ggtitle("Mean Direction for each Set") +
  xlab("Set") + ylab("Mean Direction") +
  theme_minimal()

save_output(combined_summary_plot, "mean_direction_comparison.png")
